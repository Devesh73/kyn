...
const TrendingInterestsCard = () => {
  ...
  return (
    <div className="group relative flex flex-col rounded-xl bg-slate-950 p-4 shadow-2xl transition-all duration-300 hover:scale-[1.02] hover:shadow-indigo-500/20 w-full max-w-md h-96">      {/* Gradient overlay */}
      ...
      <div className="relative h-64 overflow-y-auto space-y-4 rounded-lg bg-slate-900/50 p-3 scrollbar-thin scrollbar-thumb-indigo-600 scrollbar-track-slate-800 scrollbar-rounded-lg pr-2">
        {trendingInterests.length ? (
          <ul className="space-y-4">            {trendingInterests.map((item, index) => (
